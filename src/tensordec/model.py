...
class COMPParafac:
    ...
    def optimize(self, tensor, num_step, logging_step, l2_weight=1e-8, min_nu=0.5):
        ...
        calc_feature_operation = (
            ",".join([target[i] + "r" for i in range(self.num_axis - 1)])
            + "->"
            + target[:-1]
            + "r"
        )
        calc_lmd_operation_lst = [
            "".join([target[i] for i in range(self.num_axis) if i != t])
            + "r,"
            + target[t]
            + "r->"
            + target
            for t in range(self.num_axis)
        ]
        calc_weight_operation_lst = [
            target
            + ","
            + "".join([target[i] for i in range(self.num_axis) if i != t])
            + "r"
        ...
        def calc_zs_step(j, r_z, r_zd1, r_zd2, zd1_z, zd2_z, lmd, nu):
            ...
            return (
                ...
                zd1_z,
                zd2_z,
                jnp.max(zd1_z_rate) + jnp.max(zd2_z_rate),
            )
        ...
        def calc_zs(lmd, nu, max_step=1000000, truncate_tol=1e-10):
            ...
            for j in range(begin, max_step):
                r_z, r_zd1, r_zd2, zd1_z, zd2_z, rate_max = calc_zs_step(
                    j, r_z, r_zd1, r_zd2, zd1_z, zd2_z, lmd, nu
                )
        ...
        def calc_lmd_x(t, matrix_list):
            x = jnp.einsum(
                calc_feature_operation,
                *[matrix_list[_] for _ in range(len(matrix_list)) if _ != t],
            )
        ...
        def calc_znu_step(j, r_z, r_zd1, r_zd2, zd1_z, zd2_z, lmd, nu, factor):
            ...
            r_zd2 = (
                (j ** nu / lmd) * ((factor - jnp.log(j)) / factor) ** 2 * (1 + r_zd2)
            )
        ...
        def calc_zs_nu(lmd, nu, max_step=1000000, truncate_tol=1e-10):
            ...
            begin = 4
            for j in range(begin, max_step):
                r_z, r_zd1, r_zd2, zd1_z, zd2_z, factor, rate_max = calc_znu_step(
                    j, r_z, r_zd1, r_zd2, zd1_z, zd2_z, lmd, nu, factor
                )
                if rate_max <= (2 + truncate_tol):
                    break
            return zd1_z, zd2_z
        ...
        def calc_next_value_nu(zd1_z, zd2_z, lmd, nu):
            ...
            delta = a / w
            out = nu - delta
            out = jnp.clip(out, a_min=min_nu, a_max=10.0)
            return out
        ...
        for n_step in range(num_step):
            ...
            self.nu = step_nu(
                tensor, tuple(self.matrix_list), self.nu, max_step=self.max_step
            )
...
class ZICOMPParafac:
    def __init__(
        ...
        self.optimize(
            ...
            num_step=self.num_step,
            logging_step=self.logging_step,
            mixture_ratio_axis=mixture_ratio_axis,
            l2_weight=l2_weight,
            min_nu=min_nu,
        )
    ...
    def optimize(
        ...
        self.mixture_ratio = expectated_mixture_ratio.mean(
            axis=mixture_ratio_axis, keepdims=True
        )
        ...
        def update_mixture_ratio(mixture_ratio, matrix_list, max_step=max_step):
            likelihood = 1 / calc_z(
                jnp.einsum(self.operation, *matrix_list), self.nu, max_step=max_step
            )
            expectated_mixture_ratio = zero * (
                (mixture_ratio + 1e-8)
                / (mixture_ratio + (1 - mixture_ratio) * likelihood + 1e-8)
            )
        ...
        @partial(jax.jit, static_argnames=["t"])
        def calc_next_value(
            zd1_z, zd2_z, tensor, lmd, matrix_list, t, x, expectated_mixture_ratio
        ):
            weight = -zd2_z + zd1_z ** 2 - (tensor) / (lmd ** 2 + 1e-6)
            # weight = jnp.clip(weight, a_max=-1e-2)
            weight = jnp.clip(weight, a_max=0.0)
            weight = jnp.einsum(
                calc_weight_operation_lst[t], weight, 1 - expectated_mixture_ratio, x, x
            )
            weight = weight - jnp.expand_dims(jnp.eye(weight.shape[1]), 0) * l2_weight
            a = (
                jnp.einsum(
                    calc_a_operation_lst[t],
                    (-zd1_z + (tensor) / (lmd + 1e-6)),
                    (1 - expectated_mixture_ratio),
                    x,
                )
                - matrix_list[t] * l2_weight
            )
        ...
        def calc_next_value_nu(zd1_z, zd2_z, lmd, nu, expectated_mixture_ratio):
            a = -(
                (zd1_z - log_y_fac) * (1 - expectated_mixture_ratio)
            ).sum()  # - l2_weight*nu
        ...
        def step_nu(
            matrix_list, nu, expectated_mixture_ratio, max_step=50, truncate_tol=1e-6
        ):
            lmd = jnp.einsum(self.operation, *matrix_list)
            zd1_z, zd2_z = calc_zs_nu(
                lmd, nu, max_step=max_step, truncate_tol=truncate_tol
        ...
        for n_step in range(num_step):
            ...
            expectated_mixture_ratio = update_mixture_ratio(
                self.mixture_ratio, self.matrix_list
            )
